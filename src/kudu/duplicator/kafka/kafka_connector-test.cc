...
class KafkaConnectorTest : public KuduTest {
 public:
  ...
  uint32_t GenDecodedRowOperation(vector<kudu::RowOperationsPB>* pb_list,
    ...
    // return the length of 'type_list'
    return count;
  }
...
TEST_F(KafkaConnectorTest, ParseRow) {
  ...
  uint32_t expect_size = GenDecodedRowOperation(&pb_list, &ops);
  ASSERT_EQ(expect_size, ops.size());
