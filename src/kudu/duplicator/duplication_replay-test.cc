...
class DuplicationReplayWalTest : public tablet::TabletReplicaTestBase {
 public:
  DuplicationReplayWalTest()
      : tablet::TabletReplicaTestBase(GetSimpleTestSchema()),
        ...
        kafka_(kOffsetPort), rename_schema_(schema_) {}
  ...
  inline static Schema GetSimpleTestSchemaForColumnRename() {
    return Schema({ColumnSchema("key", INT32),
                   ColumnSchema("int_val", INT32),
                   ColumnSchema("string_val_2", STRING, true)},
                  1);
  }
 ...
 protected:
  ...
  Status Insert() {
    ...
    RETURN_NOT_OK_LOG(GenWriteRequestPB(schema_.CopyWithoutColumnIds(), &write),
                      WARNING,
                      "GenWriteRequestPB status not ok");
    RETURN_NOT_OK_LOG(ExecuteWrite(tablet_replica_.get(), write), WARNING, "write not ok");
    return Status::OK();
  }

  Status InsertAfterRename() {
    tserver::WriteRequestPB write;
    RETURN_NOT_OK_LOG(
        GenWriteRequestPBForRenameColumn(rename_schema_.CopyWithoutColumnIds(), &write),
        WARNING,
        "GenWriteRequestPB status not ok");
    RETURN_NOT_OK_LOG(ExecuteWrite(tablet_replica_.get(), write), WARNING, "write not ok");
    return Status::OK();
  ...
  int Scan(bool after_alter = false) {
    Schema projection;
    if (after_alter) {
      projection = rename_schema_.CopyWithoutColumnIds();
    } else {
      projection = schema_.CopyWithoutColumnIds();
    }
  ...
  Status GenWriteRequestPBForRenameColumn(const Schema& schema,
    ...
    RETURN_NOT_OK(row.SetInt32("int_val", insert_counter_));
    temp.append(std::to_string(insert_counter_));
    RETURN_NOT_OK(row.SetString("string_val_2", temp));
    insert_counter_++;
  ...
  void RenameColumn() {
    ...
    builder.RenameColumn("string_val", "string_val_2");
    SchemaPB new_schema;
    SchemaToPB(builder.Build(), &new_schema);
    *req.mutable_schema() = new_schema;
    req.set_schema_version(tablet()->metadata()->schema_version() + 1);

    tserver::AlterSchemaResponsePB resp;
    std::unique_ptr<tablet::AlterSchemaOpState> op_state(
        new tablet::AlterSchemaOpState(tablet_replica_.get(), &req, &resp));
    CountDownLatch latch(1);
    op_state->set_completion_callback(std::unique_ptr<tablet::OpCompletionCallback>(
        new tablet::LatchOpCompletionCallback<tserver::AlterSchemaResponsePB>(&latch, &resp)));
    ASSERT_OK(tablet_replica_->SubmitAlterSchema(std::move(op_state)));
    latch.Wait();
    rename_schema_ = GetSimpleTestSchemaForColumnRename();
  }
...
// 1. Test the ut no core dump.
// 2. Check the data correct.
TEST_F(DuplicationReplayWalTest, RenameColumnNameTest) {
  ASSERT_OK(Insert()); // NOLINT(*)
  ASSERT_EQ(1, Scan(false));
  RenameColumn();
  ASSERT_EQ(1, Scan(true));
  ASSERT_FALSE(Insert().ok());
  ASSERT_OK(InsertAfterRename()); // NOLINT(*)
  AddDuplicator();
  ASSERT_OK(InsertAfterRename()); // NOLINT(*)
  ASSERT_EQ(3, Scan(true));
  ASSERT_TRUE(tablet_replica_->consensus()->ShouldDuplication());
  tablet_replica_->StartDuplicator();
  SleepFor(MonoDelta::FromSeconds(1));
  ASSERT_OK(InsertAfterRename()); // NOLINT(*)
  int key_count;
  Consumer(4, "RenameColumnNameTest", &key_count);
  ASSERT_EQ(4, key_count);
  ASSERT_EQ(4, Scan(true));
}
