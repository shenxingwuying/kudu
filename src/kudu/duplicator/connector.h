...
struct DuplicateMsg {
  ...
  DuplicateMsg(std::shared_ptr<tablet::WriteOpState> p_op_state,
               const tablet::RowOp* p_row_op,
               std::shared_ptr<Schema> p_schema_ptr,
               std::string p_table_name)
      : op_state(std::move(p_op_state)), row_op(p_row_op),
        schema_ptr(std::move(p_schema_ptr)), table_name(std::move(p_table_name)) {
...
class KuduLocalConnector : public LocalConnector {
 public:
  KuduLocalConnector() {}
  ~KuduLocalConnector() override = default;
  Status Init() override {
    LOG(INFO) << "KuduLocalConnector Init()";
    return Status::OK();
  }
  Status WriteBatch(const std::vector<std::shared_ptr<DuplicateMsg>>& /* msgs */) override {
