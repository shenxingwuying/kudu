...
DEFINE_bool(allow_create_trash_prefix_table, false,
            "Allow creating tables with trash tag prefix. Only for testing.");
TAG_FLAG(allow_create_trash_prefix_table, hidden);
...
// Validate a table name to ensure that it is a valid identifier.
Status ValidateTableName(const string& table_name, bool external_request) {
  if (PREDICT_TRUE(!FLAGS_allow_create_trash_prefix_table)) {
    if (external_request && HasPrefixString(table_name, Master::kTrashedTag)) {
      return Status::InvalidArgument(Substitute(
          "table name should not start with reserved key word '$0'", Master::kTrashedTag));
    }
  }

  return ValidateIdentifier(table_name);
}
...
Status ValidateClientSchema(const optional<string>& name,
  ...
  if (name) {
    RETURN_NOT_OK_PREPEND(ValidateTableName(name.get(), external_request), "invalid table name");
  }
  if (owner) {
...
Status CatalogManager::CreateTable(const CreateTableRequestPB* orig_req,
  ...
  RETURN_NOT_OK(SetupError(ValidateClientSchema(
                               normalized_table_name, req.owner(), client_schema, true),
...
Status CatalogManager::AlterTable(const AlterTableRequestPB& req,
  ...
  *resp->mutable_table_id() = table->id();
  ...
  RETURN_NOT_OK(SetupError(
        ValidateClientSchema(none, none, new_schema, external_request),
        resp, MasterErrorPB::INVALID_SCHEMA));
  ...
  if (req.has_new_table_name()) {
    ...
    RETURN_NOT_OK(SetupError(
        ValidateTableName(req.new_table_name(),
                          external_request).CloneAndPrepend("invalid table name"),
        resp, MasterErrorPB::INVALID_SCHEMA));
  ...
  // 7. Alter table's replication factor.
