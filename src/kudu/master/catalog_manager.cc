...
Status CatalogManager::Init(bool is_first_run) {
  ...
  // Leader rebalancer depend on a good replicas topology, that means we'd better enable
...
void CatalogManager::GetTableInfo(const string& table_id, scoped_refptr<TableInfo> *table) {
  leader_lock_.AssertAcquiredForReading();
