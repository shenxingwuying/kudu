...
struct Redo {
  ...
  string ToString() const {
    ...
    return strings::Substitute("{$0 key=$1 val=$2}",
                               rtype == RedoType::INSERT ? "INSERT" : "UPDATE", key,
  ...
  for (const TestOp& test_op : test_ops) {
    ...
    switch (test_op.type) {
      ...
      case TEST_UPDATE_IGNORE: {
        ...
        FindOrDie(pending_redos_per_txn, kNoTxnId)
            .emplace_back(RedoType::UPDATE, row_key, latest_update.val);
      ...
      case TEST_DELETE_IGNORE: {
        ...
        FindOrDie(pending_redos_per_txn, kNoTxnId)
            .emplace_back(RedoType::DELETE, row_key, boost::none);
        auto& pending_row_by_key =
            LookupOrEmplace(&pending_vals_per_txn, kNoTxnId, ValueByRowKey{});
