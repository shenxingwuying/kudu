...
TabletServiceImpl::TabletServiceImpl(TabletServer* server)
  ...
  scan_token_ = server_->scan_pool()->NewToken(ThreadPool::ExecutionMode::CONCURRENT);
}
...
void TabletServiceImpl::Scan(const ScanRequestPB* req,
  ...
  std::shared_ptr<AsyncScanner> scan_task = std::make_shared<AsyncScanner>();
  scan_task->set_threadpool_token(scan_token_.get());
  scan_task->InitWeakPtr(scan_task);

  if (req->has_new_scan_request()) {
    ScanContext* scan_context =
        new ScanContext(seda::Action::kTServerNewScan, scan_task->GetSharedPtr());
    scan_context->req = req;
    scan_context->resp = resp;
    scan_context->rpc_context = context;
    scan_context->server = server_;
    scan_context->service = this;
    TRACE("async exec new scanner");
    // scan_task->thread_pool_token()->Execute(scan_context);
    server_->scan_pool()->Execute(scan_context);
    // scan_task->OnNewScan(scan_context);
  } else if (req->has_scanner_id()) {
    ScanContext* scan_context =
        new ScanContext(seda::Action::kTServerContinueScan, scan_task->GetSharedPtr());
    scan_context->req = req;
    scan_context->resp = resp;
    scan_context->rpc_context = context;
    scan_context->server = server_;
    scan_context->service = this;
    TRACE("async exec continue scanner");
    // scan_task->thread_pool_token()->Execute(scan_context);
    server_->scan_pool()->Execute(scan_context);
    // scan_task->OnContinueScan(scan_context);
  } else {
    context->RespondFailure(
        Status::InvalidArgument("Must pass either a scanner_id or new_scan_request"));
    return;
  }
}
...
Status TabletServiceImpl::HandleNewScanRequest(TabletReplica* replica,
  ...
  VLOG(0) << "DUYUQI MARK: " << "new scanner, call_seq_id: " << scanner->call_seq_id();
...
Status TabletServiceImpl::HandleContinueScanRequest(const ScanRequestPB* req,
  ...
  VLOG(0) << "DUYUQI MARK: " << "continue scanner, call_seq_id: " << req->call_seq_id();
