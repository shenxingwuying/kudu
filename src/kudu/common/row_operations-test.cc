...
class RowOperationsTest : public KuduTest {
 ...
 protected:
  ...
  void DoFuzzTest(const SchemaRefPtr& server_schema,
                  const KuduPartialRow& row,
                  int n_random_changes);

  SchemaRefPtr schema_;
  SchemaRefPtr schema_without_ids_;
  Arena arena_;
};
...
void RowOperationsTest::DoFuzzTest(const SchemaRefPtr& server_schema,
                                   const KuduPartialRow& row,
                                   int n_random_changes) {
  for (int operation = 0; operation <= 11; operation++) {
    ...
    CheckDecodeDoesntCrash(*client_schema, *server_schema.get(), pb);
    ...
    for (int i = 0; i < pb.rows().size(); i++) {
      ...
      CheckDecodeDoesntCrash(*client_schema, *server_schema.get(), mutated);
    }
...
TEST_F(RowOperationsTest, FuzzTest) {
  ...
  KuduPartialRow row(schema_without_ids_.get());
...
// and server schemas.
SchemaRefPtr GenRandomSchema(bool with_ids) {
  SchemaBuilder builder;
  if (random() % 5 != 0) {
...
TEST_F(RowOperationsTest, SchemaFuzz) {
  ...
  for (int i = 0; i < n_iters; i++) {
    ...
    SchemaRefPtr client_schema = GenRandomSchema(false);
    SchemaRefPtr server_schema = GenRandomSchema(true);
    KuduPartialRow row(client_schema.get());

    // On a crash or ASAN failure, dump the case information to the log so we
    // can write a more specific repro.
    g_failing_case.client_schema = client_schema.get();
    g_failing_case.server_schema = server_schema.get();
...
TEST_F(RowOperationsTest, TestFuzz1) {
  ...
  SchemaRefPtr client_schema = client_schema_builder.BuildWithoutIds();
  ...
  SchemaRefPtr server_schema = server_schema_builder.Build();
  KuduPartialRow row(client_schema.get());
...
string TestProjection(RowOperationsPB::Type type,
                      ...
                      const SchemaRefPtr& server_schema) {
  RowOperationsPB pb;
  RowOperationsPBEncoder enc(&pb);
  ...
  Arena arena(1024);
  vector<DecodedRowOperation> ops;
  RowOperationsPBDecoder dec(&pb, client_row.schema(), server_schema.get(), &arena);
  ...
  CHECK_EQ(1, ops.size());
  return ops[0].ToString(*server_schema.get());
}
...
TEST_F(RowOperationsTest, ProjectionTestWholeSchemaSpecified) {
  SchemaRefPtr client_schema(new Schema({ ColumnSchema("key", INT32),
                         ColumnSchema("int_val", INT32),
                         ColumnSchema("string_val", STRING, true) },
                       1));
  // Test a row missing 'key', which is key column.
  {
    KuduPartialRow client_row(client_schema.get());
  ...
  // Force to set null on key column.
  {
    ...
    KuduPartialRow client_row(client_schema.get());
    ContiguousRow row(client_schema.get(), client_row.row_data_);
...
TEST_F(RowOperationsTest, ProjectionTestWithDefaults) {
  ...
  SchemaRefPtr server_schema = b.Build();
...
TEST_F(RowOperationsTest, ProjectionTestWithClientHavingInvalidSubset) {
  ...
  SchemaRefPtr client_schema = b.BuildWithoutIds();
...
TEST_F(RowOperationsTest, TestProjectUpdates) {
  SchemaRefPtr client_schema(new Schema({ ColumnSchema("key", INT32),
                         ...
                         ColumnSchema("string_val", STRING, true) },
                       1));
  SchemaRefPtr server_schema = SchemaBuilder(*client_schema.get()).Build();
...
TEST_F(RowOperationsTest, TestProjectUpdatesReorderedColumns) {
  SchemaRefPtr client_schema(new Schema({ ColumnSchema("key", INT32),
                         ...
                         ColumnSchema("int_val", INT32) },
                       1));
  SchemaRefPtr server_schema(new Schema({ ColumnSchema("key", INT32),
                         ColumnSchema("int_val", INT32),
                         ColumnSchema("string_val", STRING, true) },
                       1));
  server_schema = SchemaBuilder(*server_schema.get()).Build();
...
void TestSplitKeys(bool use_tablet_schema_as_client_schema) {
  SchemaRefPtr client_schema = new Schema({ ColumnSchema("int8", INT8),
  ...
  SchemaRefPtr schema = client_schema->CopyWithColumnIds();
  RowOperationsPBDecoder decoder(&pb, use_tablet_schema_as_client_schema ? schema.get() :
                                 client_schema.get(), schema.get(), nullptr);
  vector<DecodedRowOperation> ops;
...
void CheckExceedCellLimit(const SchemaRefPtr& client_schema,
  ...
  SchemaRefPtr schema = client_schema->CopyWithColumnIds();
  RowOperationsPBDecoder decoder(&pb, client_schema.get(), schema.get(), &arena);
  vector<DecodedRowOperation> ops;
  Status s;
  switch (op_type) {
...
void CheckInsertUpsertExceedCellLimit(const SchemaRefPtr& client_schema,
                                      const vector<string>& col_values,
                                      const Status& expect_status,
                                      const string& expect_msg) {
...
void CheckUpdateDeleteExceedCellLimit(const SchemaRefPtr& client_schema,
                                      const vector<string>& col_values,
                                      const Status& expect_status,
                                      const string& expect_msg) {
...
void CheckWriteExceedCellLimit(const SchemaRefPtr& client_schema,
                               const vector<string>& col_values,
                               const Status& expect_status,
                               const string& expect_msg) {
...
void CheckSplitExceedCellLimit(const SchemaRefPtr& client_schema,
                               const vector<string>& col_values,
                               const Status& expect_status,
                               const string& expect_msg) {
...
TEST_F(RowOperationsTest, ExceedCellLimit) {
  SchemaRefPtr client_schema = new Schema({ ColumnSchema("key_string", STRING),
