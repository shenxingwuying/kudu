...
Status DiskRowSet::NewMajorDeltaCompaction(const vector<ColumnId>& col_ids,
  ...
  SchemaRefPtr schema = rowset_metadata_->tablet_schema();
...
Status DiskRowSet::NewCompactionInput(const SchemaRefPtr& projection,
...
Status DiskRowSet::DebugDump(vector<string> *lines) {
  ...
  SchemaRefPtr schema_ptr = rowset_metadata_->tablet_schema();
  RETURN_NOT_OK(NewCompactionInput(schema_ptr.get(),
