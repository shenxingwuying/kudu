...
DuplicationOp::DuplicationOp(unique_ptr<DuplicationOpState> state,
    ...
    : Op(type, Op::DUPLICATION_OP), state_(std::move(state)),
    last_confirmed_opid_(std::move(last_confirmed_opid)), dup_info_(std::move(dup_info)) {}
...
Status DuplicationOp::Prepare() {
  TRACE("PREPARE DuplicationOp: Starting");
  // Do nothing.
  TRACE("PREPARE DuplicationOp: finished");
  return Status::OK();
}
...
void DuplicationOp::Finish(OpResult result) {
  ...
  TRACE("DuplicationOpCallback: making alter schema visible");
  state()->Finish();
}
