  ...
  RowBlockRow NewRow() {
    ...
    if (row_idx == 0) {
      duplicated_rows_.push_back(std::unique_ptr<RowBlock>(
          new RowBlock(schema_.get(), kDuplicatedRowsPerBlock, static_cast<RowBlockMemory*>(nullptr))));
    }
...
Status ReupdateMissedDeltas(const IOContext* io_context,
  ...
  vector<CompactionInputRow> rows;
  const SchemaPtr schemaPtr = input->schema();
  const Schema* schema = schemaPtr.get();
