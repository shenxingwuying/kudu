...
TabletMetadata::TabletMetadata(FsManager* fs_manager, string tablet_id,
                               ...
                               SchemaRefPtr schema, PartitionSchema partition_schema,
                               Partition partition,
...
Status TabletMetadata::LoadFromSuperBlock(const TabletSuperBlockPB& superblock) {
...

  {
    ...
    SchemaRefPtr schema(new Schema());
...
void TabletMetadata::SetSchema(SchemaRefPtr schema, uint32_t version) {
  std::lock_guard<LockType> l(data_lock_);
  SetSchemaUnlocked(schema, version);
}

void TabletMetadata::SetSchemaUnlocked(SchemaRefPtr schema, uint32_t version) {
