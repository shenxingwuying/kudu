...
MajorDeltaCompaction::MajorDeltaCompaction(
    ...
    : fs_manager_(fs_manager),
      base_schema_(std::make_shared<Schema>(base_schema)),
      partial_schema_(std::make_shared<Schema>()),
...
Status MajorDeltaCompaction::Compact(const IOContext* io_context) {
  ...
  RETURN_NOT_OK(base_schema_->CreateProjectionByIdsIgnoreMissing(column_ids_, partial_schema_.get()));
