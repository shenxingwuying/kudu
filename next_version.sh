...
git config --global user.email zhangqianqiong@sensorsdata.cn
git config --global user.name qqzhang
git add ${base_path}/build-tools/package.yml
git add ${base_path}/bite-kudu/build-tools/package.yml
git commit -m "[IMPALA-74] update develop version to ${next_version}"
...
lab merge-request -b origin/dev/automate/${next_version} -t origin/develop/1.12.x -m "[KUDU-15] update develop version to ${next_version}" -a zhangqianqiong
